    ...
    ...(IS_WSL ? (() => {
      ...
      try {
        return readdirSync('/mnt/c/Users')
          .filter(u => !WSL_SYSTEM_USERS.has(u))
          .flatMap(u => [
            resolve('/mnt/c/Users', u, 'AppData/Local/Google/Chrome/User Data/DevToolsActivePort'),
          ]);
      } catch { return []; }
    })() : []),
  ].filter(Boolean);
